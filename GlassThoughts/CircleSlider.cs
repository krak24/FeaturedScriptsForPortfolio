...
public class CircleSlider : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler, IInitializePotentialDragHandler
{
    ...
    public bool hasLight = false; // Flaga okreœlaj¹ca, czy opcja 'hasLight' jest zaznaczona
    public GameObject objectToActivate; // Obiekt, który ma byæ aktywowany
    ...
    private float sliderValue; // Wartoœæ knoba w skali 0-100
    ...
    private void InvokeEvents(float value)
    {
        ...
        // Przeliczanie wartoœci knoba na skalê od 0 do 100
        sliderValue = value * 100f;

        OnValueChanged?.Invoke(sliderValue);

        // Aktywacja obiektu, jeœli hasLight jest zaznaczone i sliderValue jest wiêksze ni¿ 10
